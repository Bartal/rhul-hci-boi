...
function converMessage(message, direction) {
    var orginal = $(message).copy();
    var text = $(message).data('text');

    $(message).html('<a href="#" class="list-group-item"></a>');
    $(message).addClass($(orginal).attr('class'));
    if (direction == "SENDING") {
        $(message).addClass('text-right');
    } else if (direction == "RECIVING") {
        $(message).addClass('text-left');
    }

    var messageContainer = "<div class=\"h3 text-justify text\"><div class='text'>" +
        text +
        "</div></div>";

    if (direction == "SENDING") {
        messageContainer += "<span class=\"small\">";

        if ($(message).data(to) == "0") {
            messageContainer += "Broadcast Message";
        } else {
            messageContainer += "Send to " + $(message).data('to');
        }

        messageContainer += "</span>";
    } else if (direction == "RECIVING") {
        messageContainer += "<span class=\"small\">";
        messageContainer += "from " + $(message).data('from');
        messageContainer += "</span>";
    }
}
    ...
    $.fn.BYOIMessageHandler = function (options) {
        ...
        return this.each(function () {
            ...
            if (options.hasOwnProperty('accept')) {
                ...
                mh.data('accept', function (message) {
                    return true;
                });
            }
